...
@HiltViewModel
class EditProfileActivityVM @Inject constructor(private val apiRepoImpl: ApiRepositoryImpl) : ViewModel() {

    var userBean = MyApplication.instance?.getUserData() ?: UserBean()
    val isLoading = mutableStateOf(false)
    var isImageChange = mutableStateOf(false)

    /**
     *  Create account
     * **/
    private val _obrUpdateAccount = MutableLiveData<Resource<ApiResponse<UserBean>>>()
    val obrUpdateAccount: LiveData<Resource<ApiResponse<UserBean>>> get() = _obrUpdateAccount
    ...
    fun callCreateAccountApi() = viewModelScope.launch {
        ...
        apiRepoImpl.updateProfile(rqMap).collect {
            _obrUpdateAccount.postValue(it)
        }
    }
